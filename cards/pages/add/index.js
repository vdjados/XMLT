...
export class AddCardPage {
    ...
    render() {
        ...
        document.getElementById('add-card-form').addEventListener('submit', (e) => {
            ...
            ajax.post(creditUrls.createCredit(), newCard, (data, status) => {
                const msgBox = document.getElementById('response-message');

                if (status === 201 || status === 200) {
                    msgBox.innerHTML = '<div class="alert alert-success">Карточка добавлена успешно!</div>';
                    document.getElementById('add-card-form').reset();
                } else {
                    msgBox.innerHTML = `<div class="alert alert-danger">Ошибка при добавлении карточки (код ${status})</div>`;
                }
            });
        });
